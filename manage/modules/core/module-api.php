...
$install_module_policy->define(
    new Priv\PrivModules( settings: true, view: true, install: true )
);
AdminApi::register_endpoint(new ApiEndPoint(
    module          : "core",
    ...
    method : function(AdminApi $Api, array $args, ApiEndPoint $Endpoint) {
        ...
        $cleanup = function($installer, $delete = "", $clean = false) {
            $installer->close_zip();
            if (!empty($delete)) BsikFileSystem::delete_files($delete);
        ...
        try {
            ...
            $Installer = new ModuleInstaller($ret, Std::$fs::path_to("modules")["path"]);
            ...
            if (array_key_exists($args["module_name"], $get_module->answer_data())) {
                ...
                try {
                    $module_obj = Modules::initiate_module(
                        ...
                        db : $Api::$db,
                        Api : $Api,
                        Page : APage::$ref, // we don't really care about APage
                ...
                $temp_zip = BsikZip::zip_folder(
                    $module_obj->path, 
                    Std::$fs::path_to("trash", $trash_name)["path"]
                );

                //Delete folder:
                BsikFileSystem::clear_folder($module_obj->path ,true);
...
$get_settings_policy->define(
    new Priv\PrivModules( settings: true, view: true )
);
AdminApi::register_endpoint(new ApiEndPoint(
    module      : "core",
