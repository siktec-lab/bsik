    ...
    public function load_global(string $endpoints_path) : bool {
        ...
        if (Std::$fs::file_exists("modules", [$module, "module-api.php"])) {
            ...
            try {
                ...
                if (!APage::$modules->is_installed($module)) { 
                ...
                self::set_temp_force_global(
                    state  : true, 
                    module : !self::$front_exposed ? APage::$module->module_name : "#front#" // This check is here because we might get here from front and in this case no module check is done to bypass global restrictions
                );
                ...
                self::log("warning", $t->getMessage(), [
                    ...
                    "installed-modules" => implode(",", APage::$modules->get_all_installed()),
                    "registered-modules" => implode(",", APage::$modules->get_all_registered()),
                ]);
            }
        }
        return false;
